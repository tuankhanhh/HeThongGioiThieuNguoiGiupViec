...
namespace MyWebApi.Controllers
{
    ...
    public class KhieuNaiController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetComplaintHistory()
        {
            ...
            // Truy vấn bảng KhieuNai và join qua NgayLamViec -> DonDatDichVu để lấy MaDon
            var rawComplaints = await _context.KhieuNais
                .Include(k => k.MaNgayLamViecNavigation)
                    .ThenInclude(n => n.MaDonDatDichVuNavigation) // Truy cập vào chi tiết đơn đặt dịch vụ chứa MaDon
                .Where(k => k.MaKhachHang == customerId)
                .OrderByDescending(k => k.ThoiGian) // Mới nhất lên đầu
                .ToListAsync();

            var mappedComplaints = rawComplaints.Select(k => new KhieuNaiDTO
            {
                MaKhieuNai = k.MaKhieuNai,
                MaNgayLamViec = k.MaNgayLamViec, // Thêm dòng này để lấy mã ngày làm việc
