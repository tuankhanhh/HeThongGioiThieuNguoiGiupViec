...
import ServiceSelectionClient from "@/components/componentsCustomer/ServiceSelectionClient";
...
// Hàm fetch data chạy hoàn toàn trên Node.js Server
async function getServices(): Promise<Service[]> {
  try {
    const res = await fetch(`${process.env.API_URL}/dichvu`, {
      next: { revalidate: 60 }, // Cache lại 60 giây để tăng tốc độ phản hồi
    });

    if (!res.ok) {
      throw new Error("Lỗi fetch dịch vụ từ Backend");
    }

    return await res.json();
  } catch (error) {
    console.error("Lỗi SSR:", error);
    return []; // Trả về mảng rỗng để không crash trang nếu server backend đang sập
  }
}

// COMPONENT SERVER MAC ĐỊNH
